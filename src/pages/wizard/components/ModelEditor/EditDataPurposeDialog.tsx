...
export interface Props {
  ...
  onSave: ( label: string[], info: string, infoTitle: string, shortInfo: string[],
    typeId: string, itemSymbols: ItemSymbol[], tutorialStep: TutorialStep ) => void;}

const EditDataPurposeDialog: React.FC<Props> = props => {

  const [label, setLabel] = useState<string[]>( props.processedDataPurpose.dataPurpose.label );
  const [info, setInfo] = useState<string>( props.processedDataPurpose.dataPurpose.info );
  const [infoTitle, setInfoTitle] = useState<string>( props.processedDataPurpose.dataPurpose.infoTitle );
  const [shortInfo, setShortInfo] = useState<string[]>( props.processedDataPurpose.dataPurpose.shortInfo);
  const [typeId, setTypeId] = useState<string>( props.processedDataPurpose.dataPurpose.typeId );

  const [itemSymbols, setItemSymbols] = useState<ItemSymbol[]>( props.processedDataPurpose.dataPurpose.itemSymbols || [] );

  const currentTutorialStep = props.model.tutorial.steps.find(
    ( tutorialStep: TutorialStep ) => tutorialStep.itemId === props.processedDataPurpose.dataPurpose.id );
  ...
  return (
    <Dialog>
      <DialogHeader>Verwendungszweck bearbeiten</DialogHeader>
      <DialogBody>
        ...
        <TwoColumnContainer>
          <div>
            ...
            <TextareaInput label={""}
                           value={label.reduce((acc, current) => {return acc + "\n" + current})}
          ...
          <div>
            <PreviewItem modelState={props.model}
                         ...
                         previewItem={{
                             ...props.processedDataPurpose,
                             dataPurpose: {
                               ...props.processedDataPurpose.dataPurpose,
                               label: label,
                               shortInfo: shortInfo
                             }
                           }}
                           ...
                           onUpdate={( value: boolean ) => {
                             if( !!tutorialStep ) {
                               ...
                               setTutorialStep( {
                                 ...
                                 text: "",
                                 itemId: props.processedDataPurpose.dataPurpose.id
                               } );
                             }
                           }}
            />
